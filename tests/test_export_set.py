...
* the manifest set id (``react-grundlagen-from-de``) resolves the same
  set as the path basename slug (``react-grundlagen``),
* ``--split-size`` splits a large set's export into multiple
...
KNOWN_SLUG = "react-grundlagen"
KNOWN_MANIFEST_ID = "react-grundlagen-from-de"
KNOWN_SET_DIR = REPO_ROOT / "sets" / "de" / "react-grundlagen"
...
def test_split_size_parts_concatenate_to_the_full_lesson_order(tmp_path: Path) -> None:
    source_lessons = load_source_lessons()
    created_files = run_split_export(tmp_path, 2)
